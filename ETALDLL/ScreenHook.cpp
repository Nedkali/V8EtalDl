...
void DrawLogo(void)
{
	char* scriptname = Prof.ScriptFile;

	static char img[_MAX_PATH+_MAX_FNAME] = "";
	static char version[] = "D2Etal v0.01 ";
	static int x = (CalculateTextLen(version, 0).x/2);

	if(img[0] == '\0')
		sprintf_s(img, sizeof(img), "%sversion.bmp", Vars.szScriptPath);
...
bool __fastcall DrawHook(Genhook* hook, void* argv, uint32_t argc)
{
	if((hook->GetGameState() == (ScreenhookState)(int)argv || hook->GetGameState() == Perm) &&
	  (!hook->GetIsAutomap() || (hook->GetIsAutomap() && *vpAutomapOn)))
		hook->Draw();
	return true;
}
...
void LineHook::Draw(void)
{
	...
	if(GetIsVisible() && GetX() != -1 && GetY() != -1)
	{
		uint32_t x = GetX(), y = GetY(), x2 = GetX2(), y2 = GetY2();
		POINT loc = {x, y};
		POINT sz = {x2, y2};
		if(GetIsAutomap())
		{
