...
{
	for(UnitAny* pItem = fpGetItemFromInventory(pOwner->pInventory); pItem; pItem = fpGetNextItemFromInventory(pItem))
	{
...
UnitAny* GetInvNextUnit(UnitAny* pUnit, UnitAny* pOwner, char* szName, DWORD dwClassId, DWORD dwMode)
{
	if(pUnit->dwType == UNIT_ITEM)
	{
		...
		if(pUnit->pItemData && pUnit->pItemData->pOwnerInventory && pUnit->pItemData->pOwnerInventory == pOwner->pInventory)
		{
			...
			for(UnitAny* pItem = fpGetNextItemFromInventory(pUnit); pItem; pItem = fpGetNextItemFromInventory(pItem))
			{
