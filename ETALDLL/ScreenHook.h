...
public:
	//Genhook::Genhook(Script* nowner, JSLObject* nself, uint32_t x, uint32_t y, ushort nopacity, bool nisAutomap = false,
...
public:
	...
	void Move(uint32_t nx, uint32_t ny) { SetX(GetX()+nx); SetY(GetY()+ny); }

	void SetX(uint32_t x) { Lock(); location.x = x; Unlock(); }
	void SetY(uint32_t y) { Lock(); location.y = y; Unlock(); }
	...
	uint32_t GetX(void) const { return location.x; }
	uint32_t GetY(void) const { return location.y; }
...
public:
	/*TextHook(Script* owner, JSLObject* nself, char* text, uint32_t x, uint32_t y, ushort nfont,
...
public:
	/*ImageHook(Script* owner, JSLObject* nself, const char* nloc, uint32_t x, uint32_t y, ushort ncolor,
...
public:
	/*LineHook(Script* owner, JSLObject* nself, uint32_t x, uint32_t y, uint32_t nx2, uint32_t ny2, ushort ncolor,
...
public:
	...
	void SetX2(uint32_t nx2) { Lock(); x2 = nx2; Unlock(); }
	void SetY2(uint32_t ny2) { Lock(); y2 = ny2; Unlock(); }
	void SetColor(ushort ncolor) { Lock(); color = ncolor; Unlock(); }

	uint32_t GetX2(void) const { return x2; }
	uint32_t GetY2(void) const { return y2; }
	ushort GetColor(void) const { return color; }
};
...
public:
	/*BoxHook(Script* owner, JSLObject* nself, uint32_t x, uint32_t y, uint32_t nxsize, uint32_t nysize, ushort ncolor,
...
public:
	...
	void SetXSize(uint32_t nxsize) { Lock(); xsize = nxsize; Unlock(); }
	void SetYSize(uint32_t nysize) { Lock(); ysize = nysize; Unlock(); }
	void SetColor(ushort ncolor) { Lock(); color = ncolor; Unlock(); }

	uint32_t GetXSize(void) const { return xsize; }
	uint32_t GetYSize(void) const { return ysize; }
	ushort GetColor(void) const { return color; }
};
...
public:
	/*FrameHook(Script* owner, JSLObject* nself, uint32_t x, uint32_t y, uint32_t nxsize, uint32_t nysize,
			bool automap = false, Align align = Left, ScreenhookState state = Perm) :
